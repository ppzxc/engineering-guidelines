...
✓ context:plan은 입력을 4종(idea/spec/plan/diff)으로 분류하여 브랜치별로 처리할 것. diff 입력은 git:review로 redirect하고 플랜모드 진입을 금지할 것 [ADR-0043]
✓ idea 브랜치는 brainstorming(생성) → grill-me(경화) 정순을 따를 것. grill-me는 RAW_IDEA가 아닌 플랜 파일에 기록된 design/spec을 심문 대상으로 할 것 [ADR-0043]
...
✓ context:plan 디시플린 주입·구문 유효성 검사 상세는 [ADR-0032] 본문 참조. GAN 프롬프트 본문은 verification.md 1곳만 정본으로 유지할 것.
✓ context:plan은 `docs/context/{TASK_NAME}/` 4파일만 산출하며 소스코드를 편집하지 않을 것. ExitPlanMode 승인 후 동작은 기계적 전사+종료뿐 [ADR-0041]
✓ context:recall(`/context:recall`)을 사용하여 세션 재개할 것. `/resume` 슬래시는 Claude Code 내장 명령이므로 `context:recall` 스킬을 호출하지 말 것 [ADR-0044]
✓ spec 재시도 루프와 ExitPlanMode 게이트는 `context:verify-spec` 서브스킬에 위임할 것. plan/SKILL.md에 직접 구현 금지
