...
Gemini CLI에 cross-check를 위임하는 순수 wrapper 스킬. 라우팅 로직 없음. 복잡한 오케스트레이션은 `llm:auto` 담당.
...
| Sentinel | 의미 | 처리 |
|----------|------|------|
| `CLI_NOT_FOUND:gemini` | gemini 바이너리 부재 | 상위(`llm:auto`) 폴백 체인에서 다음 peer 시도. 단독 호출 시 → self-only |
| `CLI_TIMEOUT:gemini` | 300s 초과 | 상동 |
| `CLI_ERROR:gemini(exit=N)` | 비정상 종료 | 상동 |

## Step 1. 호스트 확인

`mcp__agy__*` MCP 도구 노출 → **Claude Code 호스트** → Step 2A 실행.  
그 외 → **비-Claude 호스트** → Step 2B 실행 (자기 자신이 Gemini일 가능성 → self-only로 대체).

## Step 2A. Claude Code 호스트 — SubAgent dispatch

`gemini:gemini-rescue` SubAgent 1순위 시도:
...
SubAgent 결과를 수신하면 `reviewer: gemini` 헤더와 함께 표시.
...
exit ≠ 0 → `CLI_NOT_FOUND:gemini` sentinel 발동. 이하 호출 건너뜀.
...
EXIT_CODE=$?
[ $EXIT_CODE -ne 0 ] && echo "CLI_ERROR:gemini(exit=$EXIT_CODE)"
```

## 출력 형식

`references/prompts.md`의 출력 스키마에 따라 `reviewer: gemini` 헤더로 반환.
