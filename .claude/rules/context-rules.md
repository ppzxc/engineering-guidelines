...
✓ 멀티 LLM 입력은 stdin pipe 또는 임시파일로만 전달하고 CLI 인자 직접 보간을 금지할 것 [ADR-0029]
✓ CLI 부재·sentinel(NOT_FOUND/TIMEOUT/ERROR) 시 Claude self-generate로 fallback하고 리뷰 모델명을 provenance에 기록할 것 [ADR-0029]
✓ context:plan의 외부 리뷰 스킬(workflow:planning·llm·agy) 추가 호출을 금지하고 verification-before-completion만 허용할 것 [ADR-0029]
