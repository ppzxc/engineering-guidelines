...
5. **유추 결과 확인 게이트 (interactive 전용)**: AskUserQuestion으로 선택된 폴더와 `last_updated` 타임스탬프를 제시하며 "최신 task `{TASK_NAME}` (마지막 업데이트: {last_updated})을(를) 재개할까요?" 확인. 사용자가 거부하면 목록을 보여주고 재선택. non-interactive 세션은 이 게이트를 건너뛰고 선택된 폴더로 진행한다.
