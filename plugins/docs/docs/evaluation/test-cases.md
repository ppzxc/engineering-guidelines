...
**입력 상황:**
`/docs:madr "Use Kafka for event streaming"` 호출. `docs/adr/` 디렉토리 없음.

**기대 동작:**
`docs/adr/` 디렉토리 자동 생성 후 `0001-use-kafka-for-event-streaming.md` 저장.

**금지 동작:**
`docs/adr/` 이외 경로에 저장하거나 디렉토리 없어서 에러.

**검증 포인트:**
파일이 `docs/adr/NNNN-*.md` 경로에 생성됨.
