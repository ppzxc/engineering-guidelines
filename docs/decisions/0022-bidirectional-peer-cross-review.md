...
* Bad, because 호스트가 자기 ID를 잘못 인지하면 분기 오류가 난다 — `mcp__agy__` 도구 노출 여부와 `claude` CLI PATH 존재 여부 두 시그널로 완화한다.
* Bad, because CLI 인자로 직접 diff 주입 시 shell injection 및 `ARG_MAX` 초과 위험 — SKILL.md에서 stdin pipe 사용을 명시한다.
* Bad, because 인증 만료 시 비대화형 호출이 300s hang에 빠질 위험 — SKILL.md의 pre-flight `claude --version` (3s timeout) 단계로 차단한다.
...
* 백틱·`$()`·큰따옴표를 포함한 PR diff에 대해서도 shell injection 없이 안전하게 stdin 전달되는지
* `claude logout` 등 미인증 상태에서 pre-flight가 즉시 sentinel을 발동하고 300s hang을 방지하는지
...
* Amends ADR-0021 — ADR-0021의 Claude→agy 단방향 결정은 그대로 유효하다. 본 ADR은 그 매트릭스를 Gemini/Antigravity 호스트로 확장한다.
