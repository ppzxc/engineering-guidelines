...
- **merge** — 기본 브랜치 동적 감지
- **issue** — 5가지 이슈 타입(bug, feature, chore, docs, review) 지원, 타입별 본문 템플릿 자동 적용; `clean`에서 리뷰 항목을 이슈로 변환할 때 사용
- **clean** — PR 전체 흐름 오케스트레이터; worktree 제거 시 Claude Code 로컬 설정(`.claude/settings.local.json`) 자동 보존; 리뷰 항목 이슈 발행을 `issue` 스킬에 위임
