...
**[karpathy 4원칙 subagent 가드레일]** — task 본문 paste에 다음 3원칙을 반드시 포함: [ADR-0013][ADR-0017]
- **§2 Simplicity First**: 요청 외 기능/추상화/유연성 코드 작성 금지. 200줄로 보이면 50줄 가능성 재탐색.
- **§3 Surgical Changes**: 변경된 모든 줄은 사용자 요청으로부터 직접 추적 가능해야 한다. 인접 코드 "개선", 요청 외 리팩터링, 무관한 dead code 삭제 금지.
- **§4 Goal-Driven Execution**: task에 명시된 검증 기준으로만 완료를 판정한다. "works on my machine" 금지. 검증 기준이 모호하면 BLOCKED 보고.
...
| §2 Simplicity First | 요청한 것만 최소 코드로. 추측성 추상화 금지 | S3 plan 작성 + S6 subagent 실행 |
| §3 Surgical Changes | 변경된 줄은 모두 요청에서 직접 추적 가능해야 한다 | S6 실행 |
| §4 Goal-Driven Execution | 성공 기준은 검증 가능해야 한다. "make it work" 금지 | S5 TDD 게이트 + S6 subagent 실행 |
