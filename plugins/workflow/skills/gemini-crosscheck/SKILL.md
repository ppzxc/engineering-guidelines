...
- Behavioral changes (mark items requiring TDD — `[TDD]` or `[TDD-EXEMPT: 사유]` per task, 실행 시 TDD 필수)
- Plan order: tidying tasks first, then behavioral changes
...
승인 후: "✅ Plan 완료 — 실행은 별도 스킬(예: feature-pipeline, subagent-driven-development)로 수행하세요."
...
### Planning Skip (Human Override)
다음 조건을 **모두** 충족할 때만 planning 없이 직접 실행 가능:
- 변경 대상 파일 수 ≤ 2
