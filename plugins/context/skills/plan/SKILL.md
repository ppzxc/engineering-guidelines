...
`spec.md` / `tasks.md`(체크리스트 추출) / `plan.md`(tasks 제거, Header+FileStructure 잔류) / `context.md` 생성.
`context.md` `<!-- last_updated: ... -->` 및 `Last Updated:` 실제 ISO-8601 UTC로 충전(플레이스홀더 금지).
`NO_TDD_TIDY` 미설정 시 `[S]`/`[B]` 태그 검증. 미충족 시 AskUserQuestion(태그추가/수동수정/post-hoc opt-out).
