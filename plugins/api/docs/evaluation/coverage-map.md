...
**평가 날짜:** 2026-04-04 (v0.0.10 업데이트) / Tier 태그 추가: 2026-04-06 (v0.2.0)
**README:** README.md
**스킬:** skills/restful-guidelines/SKILL.md

---

## Tier별 커버리지 요약 (v0.2.0 기준)

| Tier | 프로필 | 규칙 수 (목표) | 비고 |
|------|--------|---------------|------|
| **T1** | Essential | ~80 | 후행 도입 시 하위 호환성 파괴 위험 규칙 포함. 모든 API 첫날부터 적용 |
| **T2** | Standard | ~45 | 프로덕션 운영 편의. T1 이후 점진적 도입 |
| **T3** | Full | ~31 | 엔터프라이즈/고급 패턴. Bulk, LRO, Webhooks 등 |
| **합계** | Full | ~156 | 기존 커버리지 97.7% (217/222) 유지 |

> Tier 태그는 SKILL.md 각 규칙에 `[T1]`, `[T2]`, `[T3]`로 인라인 표기됨 [ADR-0010]
...
| 2.1-3 | ❌금지 | 리소스 경로에 동사 포함 금지 | COVERED | COVERED | ~~Minor~~ Fixed |
...
| 2.1-10 | ✅필수 | 하위 리소스 중첩은 1단계까지만 허용 | COVERED | MISSING | Minor |
