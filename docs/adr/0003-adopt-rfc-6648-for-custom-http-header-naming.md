...
* Good, because RFC 6648/BCP 178 준수로 장기적으로 표준 헤더로 승격 시 이름 충돌 없음
* Good, because 스킬 규칙과 예시가 일관됨 — 자기 모순 해소
* Good, because `Idempotency-Key`가 IETF 드래프트와 즉시 호환
* Bad, because `X-` 접두사에 익숙한 개발자에게 초기 혼란 가능 — 명시적 RFC 인용으로 완화

### Confirmation

`plugins/api/skills/restful-guidelines/SKILL.md` 내 모든 커스텀 헤더 예시가 `X-` 접두사 없이 표기되며, RFC 7239 예외 목록이 명시되어 있는지 확인한다.

## Pros and Cons of the Options

### Option A: `X-` 접두사 유지

* Good, because 관행적으로 익숙한 패턴
* Bad, because RFC 6648을 직접 위반
* Bad, because 스킬 자체 규칙("Custom headers omit X- prefix")과 모순

### Option B: `X-` 접두사 제거 (RFC 6648 준수)

* Good, because RFC 6648/BCP 178 완전 준수
* Good, because 신규 프로젝트이므로 마이그레이션 비용 제로
* Good, because IETF 드래프트(`Idempotency-Key`) 및 표준 정렬
* Bad, because 일부 레거시 문서나 서드파티 예시와 불일치 가능 — 예외 목록으로 관리
...
* [RFC 7239](https://datatracker.ietf.org/doc/html/rfc7239) — Forwarded HTTP Extension (`X-Forwarded-For` 예외 근거)
