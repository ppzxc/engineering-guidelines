...
Read `.context-map.md` if it exists (generated by `workflow:gemini-crosscheck`). Retrieve PR diff via `gh pr diff <PR_NUMBER>`.
...
- `prompt`:
  ...
  ```
- `context`: `.context-map.md` 내용(존재하는 경우) + `\n\nPR Diff:\n` + `gh pr diff <PR_NUMBER>` 출력
- `model`: `"gemini-3.1-pro-preview"`

**Fallback chain:**
1. 위 파라미터로 호출. **동기 실행** — 결과를 받기 전 Step 5b로 넘어가지 않는다.
2. 실패 시: notify user "⚠️ Gemini Pro → Flash fallback", 동일 파라미터에 `model: "gemini-3-flash-preview"`로 재시도. Flash 결과는 덜 상세할 수 있으므로 critical/major 항목은 소스 직접 확인 후 포함.
3. Flash도 실패 시: notify user "⚠️ Gemini unavailable, Claude-only review", Step 5b를 스킵하고 Step 5 결과만으로 진행.
