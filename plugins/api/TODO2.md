...
### 🔴 Critical (업계 4~5곳 이상 보유, 즉시 누락이 체감되는 항목)
...
| M-3 | **Partial Response 심화 (`fields=`)** | 4/6 | 쉼표 구분 필드 선택, 중첩 표기(`user.name`), 응답 예시 |
| M-4 | **Expand/Embed 심화** | 3/6 | `?expand=author,tags` 형식, 최대 깊이 제한, 순환 참조 방지 |
| M-5 | **재시도 가이드 (5xx 포함)** | 5/6 | 429 외 502/503/504에 대한 지수 백오프 + Retry-After 정책 |

### 🔵 Low (업계 1~2곳 보유, 선택적 완성도 항목)

| # | 항목 | 보유 업체 수 | 구현 방향 |
|---|------|-------------|-----------|
| L-1 | **Webhooks / Event 알림** | 3/6 | 이벤트 페이로드 구조, 서명 검증, 재시도 정책 |
| L-2 | **전문 검색 (`q=`)** | 3/6 | 단순 키워드 검색 vs 필터링 구분 가이드 |
| L-3 | **Health Check 엔드포인트** | 1/6 | `GET /health` → 200 `{"status":"ok"}` 패턴 |
| L-4 | **HEAD/OPTIONS 메서드** | 3/6 | HEAD(메타만 반환), OPTIONS(CORS preflight) 명시 |
