...
4. [S3] writing-plans — docs/plans/<slug>.md 작성
...
| Gate 3 | S5 완료 직후 | `ExitPlanMode` 또는 `AskUserQuestion`으로 최종 plan 승인 |
...
- **`superpowers:writing-plans` 호출 시**: `path` 인자를 `$(pwd)/docs/plans/<slug>.md` 절대경로로 전달. 상대경로·기본 경로 사용 금지.
...
4. **Goal-Driven Execution 가드레일 (karpathy §4)**: 각 `[TDD]` task가 막연한 목표("make it work")가 아닌 검증 가능한 성공 기준을 갖는가?
...
- **Surgical Changes 가드레일 (karpathy §3)**: 변경된 모든 줄은 사용자 요청으로부터 직접 추적 가능해야 한다. 인접 코드 "개선", 요청 외 리팩터링, 무관한 dead code 삭제 금지.
...
| §2 Simplicity First | 요청한 것만 최소 코드로. 추측성 추상화 금지 | S3 plan 작성 |
| §3 Surgical Changes | 변경된 줄은 모두 요청에서 직접 추적 가능해야 한다 | S6 실행 |
| §4 Goal-Driven Execution | 성공 기준은 검증 가능해야 한다. "make it work" 금지 | S5 TDD 게이트 |
