...
이 스킬은 spec·plan 작성을 핵심 가치로 둔다. **그중 spec 리뷰가 가장 중요하다** — 사람의 의도가 spec에 정확히 반영됐는지 확인하는 단계다. spec이 틀리면 완벽한 plan도 엉뚱한 결과를 만든다.

> **⛔ 터미널 계약**: 이 스킬의 **유일한 산출물**은 `docs/context/{TASK_NAME}/`의 **4개 마크다운 파일**이다. 이 스킬은 **소스코드를 절대 편집하지 않는다.** ExitPlanMode 승인 이후의 동작은 이미 Opus가 설계한 내용을 4파일로 **기계적으로 전사하고 종료**하는 것뿐이다. 구현은 별도 세션(`/context:recall` 또는 수동)의 책임이다.

---

## 실행 로그 컨벤션

각 Step 진입 시 **반드시** 아래 포맷의 헤드라인을 **대화에 직접 출력**한다 (플랜 파일 쓰기 아님).
...
브랜치 세부 단계(4-A, 4-B, 6-A 등)는 별도 헤드라인 없음.
...
**[로그]** `━━━ [Step 0/9] ARGUMENTS 파싱 및 검사 ━━━` 를 대화에 출력한다.
...
#### (0c) 입력 분류 (INPUT_TYPE 결정)

RAW_IDEA를 아래 우선순위로 분류한다. **diff 감지 시 EnterPlanMode 진입 없이 즉시 종료.**

**우선순위 1 — 명시 인자**: `/context:plan idea|spec|plan <내용>` 형식으로 호출 시 첫 토큰을 `INPUT_TYPE`으로 사용하고 나머지를 `RAW_IDEA`로 교체한다.

**우선순위 2 — 휴리스틱 (명시 인자 없을 시)**:

| 시그널 | INPUT_TYPE |
|--------|-----------|
| `diff --git` / `--- a/` / `+++ b/` 포함 | **diff** |
| `## Decision Outcome` / `status:` frontmatter / `## Considered Options` 포함 | **spec** |
| `- [ ]`가 3개 이상이거나 `## Task` 섹션 포함 | **plan** |
| 기타 (막연한 아이디어, 자연어 설명) | **idea** (default) |

**diff 처리**: 아래 메시지 출력 후 **즉시 종료** (EnterPlanMode 호출 금지):
```
입력이 code diff로 분류됐습니다. context:plan은 code diff를 처리하지 않습니다.

code diff 리뷰는 /git:review를 사용하세요.
```

이후 단계에서 `INPUT_TYPE`에 따라 분기한다:

| INPUT_TYPE | Step 4 | Step 5 grill 대상 | Step 6 writing-plans |
|------------|--------|-------------------|----------------------|
| **idea** | brainstorming 호출 → spec 생성 | 플랜 파일의 spec 심문 | 호출(생성) |
| **spec** | 입력을 spec으로 직접 기록 | 플랜 파일의 spec 심문 | 호출(생성) |
| **plan** | 입력을 spec·tasks 소스로 기록 | 플랜 파일의 plan 심문 | 생략(정규화만) |

### Step 1. 플랜모드 감지 및 자동 진입

**[로그]** `━━━ [Step 1/9] 플랜모드 감지 및 자동 진입 ━━━` 를 대화에 출력한다.
...
**[로그]** `━━━ [Step 2/9] TASK_NAME 정규화 ━━━` 를 대화에 출력한다.
...
**[로그]** `━━━ [Step 4/9] Design 생성 ━━━` 를 대화에 출력한다. (브랜치: `idea` / `spec` / `plan` 중 해당하는 것 명시)

**[디시플린] karpathy 풀 Skill 디스패치 (idea 브랜치 한정):**

`INPUT_TYPE == idea`이고 `NO_KARPATHY` 가 설정되지 않은 경우: `guideline:karpathy` 스킬을 호출하라 (정확한 등록명, 추론 금지).
brainstorming 에서 spec scope 결정 시 karpathy simplicity/surgical lens 가 적용되도록 한다.

#### 4-A. idea 브랜치 — brainstorming 호출

`INPUT_TYPE == idea`인 경우만 실행.
...
brainstorming이 산출한 spec 내용을 **플랜 파일의 spec 섹션**에 기록한다.

#### 4-B. spec 브랜치 — 입력 직접 기록

`INPUT_TYPE == spec`인 경우만 실행.

RAW_IDEA(입력된 spec 문서)를 그대로 **플랜 파일의 spec 섹션**에 기록한다. brainstorming 호출 없음.

#### 4-C. plan 브랜치 — 입력 직접 기록

`INPUT_TYPE == plan`인 경우만 실행.

RAW_IDEA(입력된 plan 문서)를 **플랜 파일의 spec·plan 소스**로 기록한다. brainstorming 호출 없음.
plan에서 추출 가능한 목표·아키텍처·tasks를 각 섹션에 분리 기록한다.

**[디시플린] 옵트아웃 기록 (전 브랜치 공통):**
...
**[로그]** `━━━ [Step 5/9] grill-me 경화 + spec 사람 리뷰 게이트 ━━━` 를 대화에 출력한다.

**grill-me 호출 (플랜 파일 spec/plan 대상):**
...
**중요**: grill-me는 RAW_IDEA(원시 입력)가 아닌 **플랜 파일에 기록된 내용을 심문**한다. idea 브랜치에서는 brainstorming이 이미 design을 만들었으므로 grill-me는 경화(stress-test) 역할을 한다.
...
(in-context 메모리만 의존하면 Step 5→8 사이 컨텍스트 compaction 시 결정 사항 소실.)
...
- plan 브랜치: **plan 내용** 요약 제시 (spec 섹션이 없으므로 plan 섹션 사용)

```
├ 승인 → Step 6으로 진행 (플랜모드 유지, Opus 계속)
└ 오류 →
    idea 브랜치: brainstorming 재호출(spec 갱신) → grill-me 재호출 → AskUserQuestion 재제시 (루프)
    spec/plan 브랜치: 사용자 피드백을 반영하여 플랜 파일 해당 섹션 보정 → grill-me 재호출(갱신된 내용 기반) → AskUserQuestion 재제시 (루프)
    ※ MAX_RETRIES = 3. **카운터를 명시적으로 추적한다**: 첫 rejection 전 `RETRY=0` 초기화,
      매 재시도 서두에 `[retry N/3]`을 출력하고 **플랜 파일에 기록**하여 컨텍스트 초기화 후에도 복구 가능하게 한다.
      `RETRY >= 3`이면 루프 중단, 미해결 항목을 Step 8의 context.md Blockers에 이관 후 진행.
    ※ 루프 중 사용자가 "취소" / "abort" / "그만" 입력 시 → 즉시 루프 탈출,
      현재 내용으로 Step 6 진행 (Blockers에 "사용자 취소" 기록)
```

### Step 6. Plan 구조화 + GAN gate (플랜모드 내 · Opus)

**[로그]** `━━━ [Step 6/9] Plan 구조화 + GAN gate ━━━` 를 대화에 출력한다.

#### 6-A. idea/spec 브랜치 — writing-plans 자동연계 (디스크 쓰기 금지 모드)

`INPUT_TYPE == idea` 또는 `INPUT_TYPE == spec`인 경우만 실행.
...
#### 6-B. plan 브랜치 — tasks 정규화

`INPUT_TYPE == plan`인 경우만 실행. writing-plans 신규 생성 없음.

플랜 파일에 기록된 기존 tasks 구조를 아래 기준으로 정규화한다:
- `## Task N` / `### Task N` 섹션 헤더 정합 확인
- `[P]` 병렬 마커 보존
- `NO_TDD_TIDY` 미설정 시: `- [ ]` 항목에 `[S]`/`[B]` 태그 누락 여부 확인. 누락 시 플랜 파일에 태그 추가.
- Phase 내 `[S]` 항목이 `[B]`보다 앞에 배치됐는지 확인. 순서 이상 시 보정.

#### 6-C. GAN cross-check (전 브랜치 공통, 순서 고정, 플랜모드 내)
...
**[로그]** `━━━ [Step 7/9] 최종 릴리스 게이트 (ExitPlanMode) ━━━` 를 대화에 출력한다.

**[하네스 제약]** `EnterPlanMode`(자동 진입, 사용자 클릭 불필요)와 달리 `ExitPlanMode`는 하네스가 플랜 파일을 제시하고 Approve/Reject 클릭을 강제한다. 완전 자동화 불가 — 파일 쓰기 안전 게이트다.
...
**[로그]** `━━━ [Step 8/9] 4파일 전사 + 검증 ━━━` 를 대화에 출력한다.
...
- Decision Log: Step 5(grill)에서 **플랜 파일에 기록한** 합의 결정 사항 + GAN provenance
...
(`context:recall`·`context:update`가 주석을 grep+`sort -r`로 활성 task를 선택하므로, 리터럴 플레이스홀더가 잔류하면 sort 오염 발생.)
...
**[로그]** `━━━ [Step 9/9] 완료 핸드오프 ━━━` 를 대화에 출력한다.
