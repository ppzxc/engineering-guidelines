...
`docs/context/{TASK_NAME}-2/`도 존재하면 `-3`, `-4`… 순서로 빈 슬롯을 찾는다 (최대 `-9`; 모두 사용 중이면 에러 출력 후 종료).
...
└ spec 오류 → 플랜모드 유지 (ExitPlanMode 미해제)
     ...
     ※ MAX_RETRIES = 3. **카운터를 명시적으로 추적한다**: 첫 rejection 전 `SPEC_RETRY=0` 초기화,
       매 재시도 서두에 `[spec-retry N/3]`을 출력하여 카운트 유지.
...
2. 실패하면 `docs/superpowers/plans/` 아래 파일명에 `{TASK_NAME}`이 포함된 최신 `.md`를 먼저 찾는다.
   없으면 가장 최신 `.md` 1개를 후보로 내용 확인 후 이동:
   `mv <found-path> docs/context/{TASK_NAME}/plan.md`
   (동시 실행 중인 /context:plan 세션이 의심되면 writing-plans를 재호출하여 직접 쓰기를 시도한다.)
3. 재확인 후 진행.
...
`NO_TDD_TIDY` 또는 `NO_KARPATHY` 설정 여부를 GAN prompt 에 컨텍스트로 전달한다.

**[디시플린] self-generate fallback 시 추가 지시:**
CLI 모두 실패하여 Claude self-generate 로 검토 시, GAN 프롬프트에 아래 단락을 포함할 것:
```
...
**Step-7 검증:**
- `grep -cE '^#{2,3} Task' docs/context/{TASK_NAME}/plan.md` == 0 (trim 완료)
