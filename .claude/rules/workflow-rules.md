...
✓ feature-pipeline의 S5 TDD 게이트에서 오류 발견 시 스스로 수정(자동 재생성)을 시도하지 말고 즉시 STOP하고 사용자에게 수정 요청할 것 (Gate 3 진입 금지) [ADR-0018][ADR-0020]
...
✓ feature-pipeline S2 진입 시점(=Gate 3 통과 후) 및 S6 시작 전 cwd가 worktree 경로와 일치하는지 `pwd && git rev-parse --show-toplevel` 로 검증할 것 [ADR-0014][ADR-0019]
...
✓ feature-pipeline plan 파일에 grill-me 대화 본문을 누적하지 않을 것 (결정/태스크/cross-check 요약만, 16KB 이하 유지) [ADR-0016][ADR-0020]
✓ feature-pipeline S6은 subagent 디스패치 직전 `test -f "$PLAN_FILE" && [ -s "$PLAN_FILE" ]` 로 plan 파일 존재·비공허성을 검증하고 실패 시 STOP할 것 [ADR-0020]
