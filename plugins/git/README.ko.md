...
| git-review | `/git-review` | PR diff 분석 및 코드 리뷰 제출 |
| git-merge | `/git-merge` | PR 안전한 squash merge |
| git-clean | `/git-clean` | PR 전체 흐름: 커밋 → PR → 리뷰 → 머지 → 정리 |
...
`git-clean`은 나머지 스킬을 순서대로 실행하는 오케스트레이터입니다:
...
- **git-review** — PR diff에서 언어를 감지하여 해당 언어별 리뷰어 스킬을 로딩; 매칭되는 리뷰어가 없으면 일반 모범 사례 기준으로 리뷰
- **git-merge** — 기본 브랜치 동적 감지
- **git-clean** — PR 전체 흐름 오케스트레이터; worktree 제거 시 Claude Code 로컬 설정(`.claude/settings.local.json`) 자동 보존; 리뷰 항목 GitHub Issue 자동 발행
